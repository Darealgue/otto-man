# Ateşli Kayma: zehir gibi fizik motoruyla düşen ateş partikülleri; ayak hizasında spawn, yere değince ateş patch'i
extends Node2D
...
const PARTICLE_COLOR := Color(1.0, 0.45, 0.15)

const FIRE_PATCH_SCENE = preload("res://effects/ground_fire_patch.tscn")
...
func _physics_process(delta: float) -> void:
	...
	for i in range(_positions.size()):
		...
		if hits.size() > 0:
			...
			continue
		# Havadayken değen düşmana 1 burn
		for enemy in enemies:
			...
			if _hit_enemies[i].get(eid, false):
				continue
			if world_pos.distance_to(enemy.global_position) <= HIT_RADIUS and enemy.has_method("add_burn_stack"):
