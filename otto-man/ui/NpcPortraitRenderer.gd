extends RefCounted
class_name NpcPortraitRenderer
## Herhangi bir köy NPC'sinin (Worker, ConcubineNPC — ikisi de aynı VillagerAppearance
## deseniyle çalışır) kendi görünümünü SubViewport ile TextureRect'e bağlar.
## ConcubinePortraitRenderer ile aynı kamera/timing mantığı; tek fark kaynak: burada
## bir Concubine resource'u değil, canlı NPC node'unun kendi sahnesi + appearance'ı kullanılır.

const _SPRITE_NAMES: Array[String] = [
	"BodySprite", "PantsSprite", "ClothingSprite", "MouthSprite", "EyesSprite",
	"BeardSprite", "HairSprite", "HatSprite",
]
...
static func render(
	...
	if portrait_rect == null or not is_instance_valid(portrait_rect):
		return
	if npc_node == null or not is_instance_valid(npc_node) or host == null or not is_instance_valid(host):
		return
	var npc_appearance: Variant = npc_node.get("appearance") if "appearance" in npc_node else null
	if npc_appearance == null:
		return
	var scene_path: String = npc_node.scene_file_path
	...
	if not ("appearance" in instance):
		viewport.queue_free()
		return
	instance.set("appearance", npc_appearance)
