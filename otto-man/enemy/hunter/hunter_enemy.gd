...
func _ready() -> void:
	...
	for ray in $TerrainDetection.get_children():
		if ray is RayCast2D:
			ray.collision_mask = 515  # Terrain (1) + Platforms (514)
			ray.enabled = true
...
func generate_patrol_points() -> void:
	...
	for x in range(-scan_width, scan_width + 1, scan_interval):
		...
		query.collision_mask = 515  # Include both terrain and platform layers
		var result = space_state.intersect_ray(query)
		...
		for height in range(step, max_height, step):
			...
			if result:
				...
				for side_offset in [-80, -40, 40, 80]:  # Added more scan points
					...
					side_query.collision_mask = 515  # Include both terrain and platform layers
...
func check_for_platform(offset: Vector2) -> bool:
	...
	query.collision_mask = 515  # Terrain (1) + Platforms (514)
	var result = space_state.intersect_ray(query)
