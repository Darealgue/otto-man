...
@export var fill_color: Color = Color(0.3, 0.7, 1.0, 0.9)  # Mavi dolum rengi
...
func _draw() -> void:
	...
	# Mavi dolum sadece yeşil bölgenin soluna kadar çizilir
	if fill_right >= success_zone_left:
		# Mavi dolum yeşil bölgeye ulaştı, sadece yeşil bölgenin soluna kadar çiz
		...
		# Mavi dolum henüz yeşil bölgeye ulaşmadı, normal çiz
		var fill_rect := Rect2(
			Vector2(fill_left, -half_h),
	...
	if font:
		...
		var hits_pos := Vector2(-hits_size.x * 0.5, -half_h - 20.0)
		# Text shadow
		draw_string(font, hits_pos + Vector2(1, 1), hits_text, HORIZONTAL_ALIGNMENT_CENTER, -1, font_size, Color(0, 0, 0, 0.7))
