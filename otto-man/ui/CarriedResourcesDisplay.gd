...
## Dünya haritasında kendi "sefer erzakı" panelini gösteriyor (bkz. WorldMapScene
## _setup_expedition_supply_hud) — aynı ekranda ikisi üst üste bindiği için burada
## gizleniyor; taşınan kaynak bilgisi artık o panelin ikinci satırında.
func _is_world_map_scene() -> bool:
	var scene := get_tree().current_scene
	if scene == null:
		return false
	return String(scene.scene_file_path).to_lower().contains("worldmap")
...
func _update_display() -> void:
	...
	# Zindan ganimeti (rusty_weapon/sky_feather/herb_bundle) artık SADECE
	# DungeonCollectiblesDisplay'de gösteriliyor — burada AYRICA göstermek aynı sayıların
	# iki yerde (bu panel + DungeonCollectiblesDisplay) birden görünmesine sebep oluyordu.
